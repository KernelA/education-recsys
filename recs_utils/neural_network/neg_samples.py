...
def compute_stat(interactions: pl.DataFrame):
    ...
    agg_stat_by_user = interactions.lazy().groupby(USER_ID_COL).agg(
        [
            ...
            pl.mean("progress").alias("mean_progress"),
        ]
...
def get_pos_neg_samples(interactions: pl.DataFrame, stat: pl.DataFrame):
    ...
    neg_samples = interactions.lazy().join(stat.lazy(), on=USER_ID_COL).filter(
        (pl.col("rating").is_not_nan() & (pl.col("rating") < pl.col("avg_user_rating"))) | (pl.col("progress") < pl.col("avg_user_rating"))).with_columns(pl.lit(0).alias("target")).select(pl.col(cols), pl.col("target")).collect()

    pos_samples = interactions.join(
        neg_samples, on=[USER_ID_COL, ITEM_ID_COL], how="anti").with_columns(pl.lit(1).alias("target"))

    pos_samples = pos_samples.vstack(neg_samples.join(
        pos_samples, on=[USER_ID_COL], how="anti").with_columns(pl.lit(1).alias("target")))
